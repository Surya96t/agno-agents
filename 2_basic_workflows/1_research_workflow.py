...
class ResearchReportGenerator(Workflow):
    ...
    web_searcher: Agent = Agent(
        ...
        Search for 4-5 sources and identify the 2-4 most authoritative and relevant ones.
        Prioritize:
    ...
    def get_search_results(
        ...
        """
        Search the web for articles on the given topic.

        Args:
            topic (str): The topic to search for.
            use_search_cache (bool): Whether to use cached search results.
            num_attempts (int, optional): Number of attempts to perform the search. Defaults to 3.

        Returns:
            Optional[SearchResults]: The search results containing relevant articles.
        """
        if use_search_cache:
            try:
    ...
    def write_research_report(
        ...
        """
        Generate a research report based on the scraped articles.

        Args:
            topic (str): The topic for the research report.
            scraped_articles (Dict[str, ScrapedArticle]): The scraped articles to include in the report.

        Yields:
            Iterator[RunResponse]: The generated research report.
        """
        logger.info("Writing research report")
        # Prepare the input for the writer agent
        writer_input = {
            "topic": topic,
            "articles": [v.model_dump() for v in scraped_articles.values()],
        }
        # Run the writer agent to yeild the response
